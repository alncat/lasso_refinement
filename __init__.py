...
class den_restraints(object):
  ...
  def __init__(self,
    ...
    self.refine_lasso = params.refine_lasso
    ...
    if self.refine_lasso:
      self.penalty = params.penalty
      self.den_lasso_proxies = self.ext.shared_den_lasso_proxy()
...
# normalize gradient
      norm_fac = 1./len(self.den_lasso_proxies)
      return self.ext.den_lasso_residual_sum(
        sites_cart,
        self.den_lasso_proxies,
        gradient_array,
        self.penalty*norm_fac)
    ...
    if not self.refine_lasso:
      ...
      print >> self.log, "%s | %s | %s | %s |" % \
        ("    atom 1     ",
         ...
         " eq edge dist  ",
         " d edge dist   ")
      for dp in self.den_lasso_proxies:
        i_seqs = dp.i_seqs
        a_xyz = sites_cart[i_seqs[0]]
        b_xyz = sites_cart[i_seqs[1]]
        distance_sq = distance_squared(a_xyz, b_xyz)
        distance = distance_sq**(0.5)
        eq_distance = distance_squared(dp.eq_edge, dp.eq_edge_reverse)
  ...
  def output_kinemage(self, sites_cart):
    ...
    f.write(vec_header)
    if not self.refine_lasso:
      proxies = self.den_proxies
    else:
      proxies = self.den_lasso_proxies
    for dp in proxies:
      i_seqs = dp.i_seqs
