...
class manager(object):
  def __init__(
    ...
    if len(den_manager.den_lasso_proxies) == 0:
      ...
      den_manager.build_den_lasso_restraints(sites_cart=self.model.xray_structure.sites_cart())
...
# currently disable neutron
    ...
    utils.assert_xray_structures_equal(
      ...
      x2 = self.model.xray_structure)
    best_penalty = penalty_local = grid_pair[0] #rho in admm iteration
    ...
    scad_weight_local = self.model.restraints_manager.geometry.\
          generic_restraints_manager.den_manager.scad_weight
...
#generate regularization path
    ...
    initial_xyz_w = self.target_weights.xyz_weights_result.w 
    alpha = 4.0
    threshold = 5e-4
    early_stopping = 3
    for i in xrange(-num_reg_den_cycles,num_reg_den_cycles):
      ...
      for scale in xrange(-4, 3):
        ...
        #self.model.restraints_manager.geometry.\
        #  generic_restraints_manager.den_manager.scad_weight = \
        #  scad_weight_local*2**scale
        trial_target_weights.xyz_weights_result.w = initial_xyz_w*1.3**scale
        pre_cycle = 0
        cycle = 0
        ...
        if best_proxies is not None:
          self.model.restraints_manager.geometry.\
            generic_restraints_manager.den_manager.den_lasso_proxies = best_proxies
        while cycle < num_den_cycles:
          ...
          minimized = minimization.lbfgs(
                ...
                use_direction            = self.params.den.use_direction,
        ...
        #synchronize model.xray_structure with xray_structure in fmodels
          ...
          if self.ncs_manager is not None:
            self.ncs_manager.update_dihedral_ncs_restraints(
              ...
              pdb_hierarchy=self.model.pdb_hierarchy(sync_with_xray_structure=True),
              log=local_log)
          if self.params.den.use_sqrt or self.params.den.use_scad:
            sites_cart = self.model.xray_structure.sites_cart()
            #sites_cart = self.model.xray_structure.unit_cell().orthogonalize(self.save_scatterers_local_try.extract_sites())
      #update the weight for each edge
            if(self.model.use_ias and self.model.ias_selection is not None and
                self.model.ias_selection.count(True) > 0):
              sites_cart = sites_cart.select(~self.model.ias_selection)
            self.model.restraints_manager.geometry.\
                generic_restraints_manager.den_manager.update_weight(sites_cart)
          cycle += 1
          self.model.restraints_manager.geometry.\
